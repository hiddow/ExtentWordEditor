...
// Initialize lazily to ensure environment variables are ready, though normally they are.
// But mostly to handle potential re-instantiation needs or error boundaries.
const getAIClient = () => {
...
export const generateVocabData = async (
  ...
  try {
    ...
    // Define the language properties structure to reuse for both translations and exampleTranslations
...
export const generateTokenData = async (
    ...
    try {
        ...
        const prompt = `
          Analyze the word/token "${token}" in ${targetLanguage}.
          Provide:
          1. script: Reading/Pinyin. **CRITICAL**: For Chinese, strictly lowercase, space-separated, with tones (e.g. "dé").
          2. variant: Traditional Chinese character (if Chinese).
          3. translation: Short English definition.
          4. translations: Definition in all requested languages.
        `;
        ...
        return {
            ...
            translations: mapKeys(data.translations)
        };
    } catch (e) {
